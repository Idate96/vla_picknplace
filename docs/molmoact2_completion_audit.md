...
3. The committed MuJoCo path covers arm physics only. It does not model the
   screwdriver object, task contacts, camera feedback, or a task-success metric.
```
...
is now committed and collaborator-reproducible as both a joint-space command
smoke and a public RobotStudio SO101 MuJoCo arm-physics smoke, but not as a full
screwdriver task-success simulator.
